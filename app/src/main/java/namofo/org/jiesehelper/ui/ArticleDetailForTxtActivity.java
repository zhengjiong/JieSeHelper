...
public class ArticleDetailForTxtActivity extends AppCompatActivity{
    ...
    protected void onResume() {
        super.onResume();
        MobclickAgent.onPageStart("txt文章详情"); //统计页面(仅有Activity的应用中SDK自动调用，不需要单独写)
    ...
    protected void onPause() {
        super.onPause();
        MobclickAgent.onPageEnd("txt文章详情"); // （仅有Activity的应用中SDK自动调用，不需要单独写）保证 onPageEnd 在onPause 之前调用,因为 onPause 中会保存信息
